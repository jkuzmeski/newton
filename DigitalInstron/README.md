...
`projects/digital_instron_v2/core.py` provides the NumPy reference prediction
used for fitting and evaluation. `projects/digital_shoe/runtime.py` implements
the live Warp foundation, and `projects/digital_instron_v2/dynamics_diff.py`
provides its differentiable calibration variant. Released columns carry no tension.
...
grid contains 910 midsole columns. The standalone exporter records the fixture
column count separately in `digital_shoe.json`; compressed-column counts depend
on the calibrated fixture pose and the displacement frame.

## Source layout

- `manifest_v2.json`: averaged traces, geometry, and initial values
- `../ASSET_PROVENANCE.md`: checksums and redistribution status for project data
