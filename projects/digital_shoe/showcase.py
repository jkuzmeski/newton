...
from .rendering import attached_column_tops, column_colors, column_world_positions
...
class Example:
    ...
    def _add_visual_plate(self, builder, *, z: float, color, label: str) -> None:
    ...
    def _build_instron(self, builder):
        ...
        self._add_visual_plate(builder, z=float(np.max(fixture.carrier_anchor_m[:, 2])) + 0.008, color=(0.65, 0.68, 0.72), label="instron_crosshead")
    ...
    def _build_drop(self, builder, args):
        ...
        hz = 0.035
        inertia = wp.mat33(
            self.drop_mass_kg * (hy * hy + hz * hz) / 3.0, 0.0, 0.0,
            0.0, self.drop_mass_kg * (hx * hx + hz * hz) / 3.0, 0.0,
            0.0, 0.0, self.drop_mass_kg * (hx * hx + hy * hy) / 3.0,
        )
        self.carrier = builder.add_link(
            xform=wp.transform_identity(),
            mass=self.drop_mass_kg,
            com=wp.vec3(0.0, 0.0, hz),
            inertia=inertia,
        ...
        joint = builder.add_joint_prismatic(
            ...
            limit_lower=-0.08,
            limit_upper=max(0.15, 2.0 * self.drop_height_m),
            limit_kd=1.0e3,
            label="vertical_guide",
        )
        builder.add_articulation([joint], label="drop_tower")
        builder.joint_q[-1] = self.drop_height_m
        self._add_visual_plate(builder, z=hz, color=(0.32, 0.38, 0.46), label="drop_mass")
    ...
    def _build_rocker(self, builder):
        ...
        self.carrier = builder.add_body(mass=1.0, com=wp.vec3(0.0), inertia=wp.mat33(np.eye(3)))
        self._add_visual_plate(builder, z=float(np.mean(bed.rest_length_m)) + 0.008, color=(0.72, 0.52, 0.28), label="rigid_rocker")
        self.foundation_config = FoundationConfig(stretch_floor=0.05)
        return (
            bed.anchor_bottom_m,
            np.zeros(len(bed.rest_length_m)),
    ...
    def render(self) -> None:
        ...
        wp.launch(column_world_positions, dim=self.column_count, inputs=[self.carrier, self.state_0.body_q, self._anchor, self._points], device=self.device)
        wp.launch(column_colors, dim=self.column_count, inputs=[self.foundation.compression, 0.010, self._colors], device=self.device)
        if self.mode == "instron":
            self.viewer.log_lines("digital_shoe/columns", self._fixed_bottom, self._points, self._colors, width=0.003)
        else:
            wp.launch(attached_column_tops, dim=self.column_count, inputs=[self._points, self._rest, self._tops], device=self.device)
    ...
    def _test_drop(self, force: np.ndarray) -> None:
        ...
        print(f"[digital shoe / drop] {self.drop_mass_kg:.1f} kg from {1000*self.drop_height_m:.0f} mm: peak {peak:.0f} N, compression {1000*compression:.1f} mm, first-contact impulse {impulse:.2f} N·s")
